...
UIButton::UIButton(float x, float y, float width, float height, 
    float scaleWidth, float scaleHeight, sf::String labelText, sf::Font font, 
    ButtonListener* listener, std::string buttonCode)
    : UIElement(
    ...
    _hoverTexture->create(width, height);
    _hoverTexture->update(hoverPixels);

    _clickTexture->create(width, height);
...
void UIButton::mouseButtonReleased(const int mx, const int my) {
    ...
    if (bounds.contains(_mx, _my)) {
        std::thread callbackThread(
            &ButtonListener::buttonPressed, _listener, _buttonCode
        );
